<svelte:head>
	<title>Privacy Policy</title>
</svelte:head>

<div class="mx-auto min-h-screen max-w-screen-xl p-4 py-18">
	<div class="mx-auto max-w-3xl">
		<h1 class="mb-8 text-4xl font-medium">Privacy Policy</h1>

		<div class="prose prose-slate max-w-none">
			<p class="text-muted-foreground mb-6">Last updated: January 16th, 2025</p>

			<h2 class="mt-8 mb-4 text-2xl font-medium">1. Introduction</h2>
			<p>
				PaulPlay ("we," "us," or "our") is committed to protecting the privacy of our users. This Privacy Policy
				explains how we collect, use, and protect information when you use our AI-powered bug reporting platform
				("Service").
			</p>

			<h2 class="mt-8 mb-4 text-2xl font-medium">2. Information we collect</h2>
			<p>2.1. Account information</p>
			<p>When you create an account, we collect:</p>
			<ul class="mb-4 list-disc pl-6">
				<li>Your name and email address (via GitHub authentication)</li>
				<li>Your GitHub username and profile picture</li>
				<li>Your subscription tier and usage information</li>
				<li>Any further information that you provide at your own choice</li>
			</ul>

			<p class="mt-4">2.2. Bug report data</p>
			<p>When you use our Service, we store data such as, but not limited to:</p>
			<ul class="mb-4 list-disc pl-6">
				<li>Bug reports, including titles, descriptions, email addresses and technical details</li>
				<li>Comments and collaboration data</li>
				<li>File uploads (images, videos, documents)</li>
				<li>System information automatically collected from your browser</li>
				<li>AI-generated suggestions and analysis</li>
			</ul>

			<p class="mt-4">2.3. Technical information</p>
			<p>We automatically collect technical information including, but not limited to:</p>
			<ul class="mb-4 list-disc pl-6">
				<li>IP addresses</li>
				<li>Browser type and version</li>
				<li>Device and operating system information</li>
				<li>Pages visited and features used</li>
				<li>Access times and dates</li>
			</ul>

			<h2 class="mt-8 mb-4 text-2xl font-medium">3. How we use your information</h2>
			<p>We use the information we collect to:</p>
			<ul class="mb-4 list-disc pl-6">
				<li>Provide and improve our bug reporting services</li>
				<li>Generate AI-powered suggestions and analysis</li>
				<li>Enable collaboration between team members</li>
				<li>Process payments and manage subscriptions</li>
				<li>Send important service updates and notifications</li>
				<li>Detect and prevent security incidents</li>
				<li>Analyze usage patterns to improve our Service</li>
			</ul>

			<h2 class="mt-8 mb-4 text-2xl font-medium">4. AI Processing</h2>
			<p>
				4.1. We use third-party AI services to analyze your bug reports and provide suggestions. Your bug report content
				may be processed by these AI services to generate improvements and recommendations.
			</p>
			<p>
				4.2. We do not train AI models on your personal data. However, we cannot control or be held responsible for how
				third-party AI providers process your bug reports once submitted to their services for analysis.
			</p>

			<h2 class="mt-8 mb-4 text-2xl font-medium">5. Data sharing and third parties</h2>
			<p>We may share your information with:</p>
			<ul class="mb-4 list-disc pl-6">
				<li>
					<strong>AI Service providers:</strong> For generating bug report improvements and suggestions, through
					<a
						href="https://openrouter.ai/privacy"
						target="_blank"
						class="hover:text-foreground underline transition-colors">OpenRouter</a
					>
				</li>
				<li>
					<strong>Payment processors:</strong>
					<a href="https://stripe.com/privacy" target="_blank" class="hover:text-foreground underline transition-colors"
						>Stripe</a
					> processes subscription payments according to their privacy policy
				</li>
				<li>
					<strong>Cloud storage:</strong>
					<a
						href="https://www.digitalocean.com/legal/privacy-policy"
						target="_blank"
						class="hover:text-foreground underline transition-colors">DigitalOcean Spaces</a
					> for secure file storage
				</li>
				<li>
					<strong>Authentication:</strong>
					<a href="https://clerk.com/privacy" target="_blank" class="hover:text-foreground underline transition-colors"
						>Clerk</a
					> for secure user authentication and management
				</li>
				<li><strong>Legal requirements:</strong> When required by law or to protect our rights</li>
				<li>
					<strong>Customers:</strong> Customers get access to the bug reports created through their forms. They will process
					these reports according to their privacy policies. Bugspot takes no accountability over how customers use & store
					their reports.
				</li>
			</ul>
			<p>We do not sell, rent, or trade your personal information to third parties for marketing purposes.</p>

			<h2 class="mt-8 mb-4 text-2xl font-medium">6. Data storage and security</h2>
			<p>6.1. We store your data securely using best security practices.</p>
			<p>6.2. Bug reports and files are stored on secure cloud infrastructure with regular backups.</p>
			<p>6.3. While we take security seriously, no method of transmission over the internet is 100% secure.</p>

			<h2 class="mt-8 mb-4 text-2xl font-medium">7. Data retention</h2>
			<p>8.1. We retain your account information for as long as your account is active and the data is needed.</p>
			<p>
				8.2. Bug reports and related data are retained until you (or we, e.g. if they violate our terms) delete them.
			</p>
			<p>8.3. Technical logs and analytics data are retained for up to 12 months.</p>
			<p>
				8.4. After account deletion, we may retain some information as required by law or for legitimate business
				purposes.
			</p>

			<h2 class="mt-8 mb-4 text-2xl font-medium">8. Your rights and choices</h2>
			<p>You have the right to:</p>
			<ul class="mb-4 list-disc pl-6">
				<li>Access your personal data</li>
				<li>Correct or update your account information</li>
				<li>Delete your bug reports and account</li>
				<li>Request information about how your data is processed</li>
			</ul>

			<h2 class="mt-8 mb-4 text-2xl font-medium">9. Children's Privacy</h2>
			<p>
				Our Service is not directed to children under 13. We do not knowingly collect personal information from children
				under 13. If you become aware that a child has provided us with personal information, please contact us
				immediately.
			</p>

			<h2 class="mt-8 mb-4 text-2xl font-medium">10. Changes to this privacy policy</h2>
			<p>
				We may update this Privacy Policy from time to time. We will notify users of any material changes by posting the
				new Privacy Policy on this page and updating the "Last updated" date. For significant changes, we may also send
				email notifications.
			</p>

			<h2 class="mt-8 mb-4 text-2xl font-medium">11. Contact Us</h2>
			<p>
				For questions about this Privacy Policy or to exercise your data rights, please contact us through the form at
				the bottom of the
				<a href="https://paulplay.studio" target="_blank" class="hover:text-foreground underline transition-colors">
					studio website
				</a>.
			</p>

			<div class="text-muted-foreground mt-12 mb-8 text-sm">
				<p>By using the Service, you acknowledge that you have read and understood this Privacy Policy.</p>
			</div>
		</div>
	</div>
</div>
